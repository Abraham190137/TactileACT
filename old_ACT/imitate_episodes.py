...
def main(args):
    ...
    # load dataset
    train_dataloader, val_dataloader, stats, _ = load_data(dataset_dir, num_episodes, camera_names, batch_size_train, batch_size_val, success_conditioned=SUCCESS_CONDITIONED)
...
def eval_bc(config, ckpt_name, save_episode=True):
    ...
    # load environment
    dt = 0.1
    env = RobotEnv(camera_numbers=camera_names, 
                   use_gelsight=True,
                   image_height=480,
                   image_width=848,
                   inject_noise=False,
    ...
    for rollout_id in range(num_rollouts):
        ...
        with torch.inference_mode():
            for t in range(max_timesteps):
                ...
                target_pose_info = action

                ### step the environment
                # print('target', target_qpos)
                obs = env.step_normalized_pose_info(target_pose_info, obs['pose'])

                ### for visualization
                qpos_list.append(qpos_numpy)
                target_qpos_list.append(target_pose_info)
