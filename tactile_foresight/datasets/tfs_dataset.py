"""
TFS 训练数据集：加载预提取 DINOv2 特征 + HDF5 中的 action chunk。

支持三种 action_mode：
  - "eef_delta":  eef_abs 帧间差分 (推荐，速度信号)
  - "eef_rel":    eef_abs 相对首帧 (累计位移)
...
每个样本:
    ...
    tactile_current: (768,) 当前触觉 DINOv2 特征
    tactile_future_seq: (H, 768) 未来 H 步触觉真值
...
_ACTION_MODE_CONFIG = {
    ...
    "eef_rel":   ("eef_abs", False),   # 相对首帧
    "joint_abs": ("joint_abs", False), # 绝对关节角
}


class TFSDataset(Dataset):
    """TFS training dataset: DINOv2 features + action chunks from HDF5."""
    ...
    def __init__(
        ...
        # max_t: ensure future tactile + action chunk don't overflow
        ...
        print("[TFSDataset] {} episodes, {} samples, horizon={}, action_mode={}, dim={}".format(
            len(self.episode_ids), len(self._index), pred_horizon, action_mode, self.action_dim))
        print("[TFSDataset] action mean={}, std={}".format(
    ...
    def _get_action_chunk(self, actions, t):
        ...
        if self.action_mode == "eef_delta":
            ...
            # relative to first frame of chunk
    ...
    def __getitem__(self, idx: int):
        ...
        # Current tactile
        tactile_current = feats["tactile"][t].float()  # (768,)

        # Future tactile sequence (ground truth)
        tactile_future = feats["tactile"][t + 1: t + 1 + self.pred_horizon].float()  # (H, 768)
        ...
        return {
            ...
            "tactile_future_seq": tactile_future,
...
def collate_tfs(batch: list) -> dict:
    """Collate function for TFSDataset."""
    return {
        "vision_feat": torch.stack([s["vision_feat"] for s in batch]),
        "tactile_current": torch.stack([s["tactile_current"] for s in batch]),
        "tactile_future_seq": torch.stack([s["tactile_future_seq"] for s in batch]),
